...
namespace UnityEditor.VFX {
    abstract class VFXAbstractParticleOutput: VFXAbstractRenderedOutput, IVFXSubRenderer {
        ...
        protected int customRenderQueue = 0;
        ...
        protected virtual bool bypassExposure {
            get {
                return true;
            }
        } // In case exposure weight is not used, tell whether pre exposure should be applied or not

        // IVFXSubRenderer interface
        public virtual bool hasShadowCasting {
            get {
                return castShadows;
            }
        }

        protected virtual bool needsExposureWeight {
            get {
                return true;
            }
        }
        ...
        private bool hasExposure {
            get {
                return needsExposureWeight && subOutput.supportsExposure;
            }
        }

        public bool HasIndirectDraw() {
            return (indirectDraw || HasSorting()) && !HasStrips(true);
        }
        public bool HasSorting() {
            return (sort == SortMode.On || (sort == SortMode.Auto && (blendMode == BlendMode.Alpha || blendMode == BlendMode.AlphaPremultiplied))) && !HasStrips(true);
        }
        ...
        public bool NeedsDeadListCount() {
            return HasIndirectDraw() && (taskType == VFXTaskType.ParticleQuadOutput || taskType == VFXTaskType.ParticleHexahedronOutput);
        } // Should take the capacity into account to avoid false positive
        ...
        public override bool codeGeneratorCompute {
            get {
                return false;
            }
        }

        public virtual bool supportsUV {
            get {
                return false;
            }
        }

        public virtual CullMode defaultCullMode {
            get {
                return CullMode.Off;
            }
        }
        public virtual ZTestMode defaultZTestMode {
            get {
                return ZTestMode.LEqual;
            }
        }

        public virtual bool supportSoftParticles {
            get {
                return !isBlendModeOpaque;
            }
        }

        private bool hasSoftParticles => supportSoftParticles && useSoftParticle;

        protected bool usesFlipbook {
            get {
                return supportsUV && (uvMode == UVMode.Flipbook || uvMode == UVMode.FlipbookBlend || uvMode == UVMode.FlipbookMotionBlend);
            }
        }
            ...
            if(supportsUV && uvMode != UVMode.Default) {
                ...
                switch(uvMode) {
                    ...
                    case UVMode.ScaleAndBias:
                        ...
                        break;
                    default:
                        throw new NotImplementedException("Unimplemented UVMode: " + uvMode);
                }
            }
        ...
        public override IEnumerable<KeyValuePair<string, VFXShaderWriter>> additionalReplacements {
            get {
                ...
                shaderTags.Write(string.Format("Tags {{ \"Queue\"=\"{0}\" \"IgnoreProjector\"=\"{1}\" \"RenderType\"=\"{2}\" }}", (customRenderQueue == 0) ? renderQueueStr : "Transparent" + customRenderQueue, renderQueueStr, !isBlendModeOpaque, renderTypeStr));
        ...
        protected virtual VFXShaderWriter renderState {
            get {
                ...
                switch(zTest) {
                    case ZTestMode.Default:
                        rs.WriteLine("ZTest LEqual");
                        break;
                    case ZTestMode.Always:
                        rs.WriteLine("ZTest Always");
                        break;
                    case ZTestMode.Equal:
                        rs.WriteLine("ZTest Equal");
                        break;
                    case ZTestMode.GEqual:
                        rs.WriteLine("ZTest GEqual");
                        break;
                    case ZTestMode.Greater:
                        rs.WriteLine("ZTest Greater");
                        break;
                    case ZTestMode.LEqual:
                        rs.WriteLine("ZTest LEqual");
                        break;
                    case ZTestMode.Less:
                        rs.WriteLine("ZTest Less");
                        break;
                    case ZTestMode.NotEqual:
                        rs.WriteLine("ZTest NotEqual");
                        break;
                }
